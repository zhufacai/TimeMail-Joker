...
while (true) {
  ...
  //读取数据库信息
  if (!empty($re)) {
    $return = emailsend($re['email'],$re['topic'],"<h2>TimeMail - 时光邮局</h2><br />您收到一封来自" . date("Y",$re['from']) . "年" . date("m",$re['from']) . "月" . date("d",$re['from']) . "日的信件：<br /><br />" . $re['content'] . "<br /><br />感谢使用<a href='".URL."'>时光邮局</a>");
    if ($return = 200) {
      $uid = $re['uid'];
      $topic = $re['topic'];
      $content = $re['content'];
